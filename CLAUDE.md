## Description
This is an online version control and file modification tool.
It's intent is to enable teams of humans and agents to share real time updates on a shared codebase.
...
* Server mode.  The process runs a web socket server that stores changes and rebroacasts to all connected clients except the sender.
* Client mode. The process runs and detects changes in the local filesystem (rooted in configurable working directory)
  * -p2p mode - the clients use the server for signalling only and all data is sent p2p over webrtc (more secure)
  * client-server mode - the clients use the server as a shared, distributed store for the data (more scalable)
...
## Key Server Features:
* Should not leak data or code.  Discovering other clients is OK.
